...
import numpy as np
from  BermudanMaxCall_model import model_Schoenmakers
...
def main(d=3, L=3, print_progress=True, steps=9,T=3, r=0.05, delta_dividend=0.1, traj_est=80000, grid=100, mode_kaggle=False, traj_test_lb=150000, traj_test_ub=10000, K_low=200, K_up=50, S_0=110, strike=100, seed=0, step_size=None, payoff_=lambda x, strike: utils.payoff_maxcal(x, strike), seed_traj_testingALL_add=0):
    """
    ...
    S = np.ones((traj, steps+1, d))*S_0_train
    S2 = np.ones((traj_test_lb, steps+1, d))*S_0
    for time in range(1,steps+1):
        S[:,time,:] = S[:,time-1,:]+S[:,time-1,:]* ((r - delta_dividend)*dt + dWS[:,time-1,:]*sigma) 
        S2[:,time,:] = S2[:,time-1,:]+S2[:,time-1,:]* ((r - delta_dividend)*dt + test_rng.normal(size=(traj_test_lb, d)).astype(np.float32)*((dt)**0.5)*sigma)
    ...
    mode_= 1 # MODE=1: INCLUDE TERMS IN REGRESSION TO FIT NOISE
    K_lower= K_low
    K_upper = K_up*d
    ...
    model_ = model_Schoenmakers(model_nn_rng, seed, input_size_lb, K_lower, steps, d, K_upper, input_size_ub, L=L, mode_kaggle=mode_kaggle)
