...
File which executes Andersen Broadie (2004) Upper bound approach to pricing a Bermudan Max Call option, with possibly multiple exercise rights.
        This variant of algorithm is implemented according to interpolation LongstaffSchwartz LSMC (keeping track of stopping value) and Tsisikilis & Van Roy LSMC (estimating previous right continuation value).
"""
...
def main(d=3, L=3, print_progress=True, steps=9,T=3, r=0.05, delta_dividend=0.1, traj_est=80000, grid=100, mode_kaggle=False, traj_test_lb=150000, traj_test_ub=10000, K_low=200, K_noise=None, S_0=110, strike=100, seed=0, payoff_=lambda x, strike: utils.payoff_maxcal(x, strike)):
    ...
    for ex_right in range(L):
        ...
        for time in range(steps)[::-1]:
            ...
            con_val = model_.train_finallayer_continuationvalue(reg_m, stop_val, time, traj, dWS[:,time,:], ex_right)
            prev_right_c = model_.prediction_conval_model1(reg_m, traj_est, time, ex_right-1) if ex_right>0 else 0
            stop_val = np.where(payoff_underlying+prev_right_c<con_val, stop_val, payoff_underlying+prev_right_c) 
 ...
 #### TESTING - UB2  Brute Force. ALTERNATIVE WAY TO COMPUTE UPPERBOUND.
    ...
    #     M_part=np.hstack([(M[:, L-ex_right_num-1, time_right_prev] -  M[:, L-ex_right_num-1, time_right_next])[:,None] for ex_right_num, [time_right_prev,time_right_next]  in enumerate(ex_rights_extended) ])
    #     M_part=np.sum(M_part, axis=1)
    #     comb_2.append((Z_part+M_part)[:,None])
    # comb_2= np.hstack((comb_2))
    # max_2 = np.max(comb_2, axis=1)
    # ub2 = np.mean(max_2)
    # ub2_std= np.std(max_2)/(len(max_2)**.5)    
    ...
    ## UPPERBOUND
    Dual_max_traj=theta_upperbound[:, -1,0] # Upperbound series
...
if __name__=='__main__':
    ...
    for d,s0,n_stopping_rights in [(2,90,2), (2,90, 1), (2, 90, 3)]:
        for grid in [500]:
