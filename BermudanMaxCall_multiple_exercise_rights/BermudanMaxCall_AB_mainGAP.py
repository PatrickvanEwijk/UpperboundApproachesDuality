...
File which executes Andersen Broadie (2004) Upper bound approach to pricing a Bermudan Max Call option, with possibly multiple exercise rights.
        This variant of algorithm is implemented according to LongstaffSchwartz LSMC.
        CORRESPONDS TO ORIGINAL APPROACH Andersen Broadie (2004).
"""
...
def main(d=3, L=3, print_progress=True, steps=9,T=3, r=0.05, delta_dividend=0.1, traj_est=80000, grid=100, mode_kaggle=False, traj_test_lb=150000, traj_test_ub=10000, K_low=200, K_noise=None, S_0=110, strike=100, seed=0, payoff_=lambda x, strike: utils.payoff_maxcal(x, strike)):
    ...
    Main function, which executes algorithm by Andersen Broadie (2004). 
        Implementation is identical to Andersen Broadie (2004): gap between lower- and upper-biased estimator is estimated together with lower-biased estimator to construct an upperbiased estimate.
    ...
    for d,s0,n_stopping_rights in [(2,90,3), (2,90, 2), (2, 90, 1)]:
        for grid in [120]:
            ...
            for i in range(1):                
                ...
                list_inf=main(d, n_stopping_rights, True, grid=grid, K_low=400,K_noise=50, traj_est=250000, traj_test_ub=10000, traj_test_lb=500000, S_0=s0, seed=i+8)
                label_='AB GAP'
