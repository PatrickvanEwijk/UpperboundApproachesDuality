...
def main(d=3, L=3, print_progress=True, steps=9, T=3, r=0.05, delta_dividend=0.1, traj_est=80000, grid=100, mode_kaggle=False, traj_test_lb=150000, traj_train_ub=10000, traj_test_ub=20000, K_low=200, K_up=100, S_0=110, strike=100, seed=0, mode_desai_BBS_BHS='desai', mean_BBS=1, std_BBS=0.0, lambda_lasso=1/100, payoff_=lambda x, strike: utils.payoff_maxcal(x, strike), seed_traj_testingALL_add=0):
    ...
    for time in range(steps):       
        ...
        traj_testing_phase=np.random.normal(size=(traj_test_ub, d, grid))
        if seed_traj_testingALL_add!=0 : # Overwrite if testing phase and using new trajectories (so final run)
            traj_testing_phase=rng_test_loopSAA.randn(traj_test_ub, d, grid)
        traj_inner_testing = np.exp( (traj_testing_phase.astype(np.float32)*((dt)**0.5)*sigma) + (r-delta_dividend-sigma**2/2)*dt)*(S4[:, time, :][:,:,None])
