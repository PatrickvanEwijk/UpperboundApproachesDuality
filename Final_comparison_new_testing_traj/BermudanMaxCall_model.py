...
class model_SAA(model_glasserman_general):
    ...
    def LP_BELOM_BFGS_multiple(self, payoff_paths, created_martingale_incrs, lambda_=3, p=50, print_progress=True, calc_nonsmoothenedTrainingUB= False, ridge_penalty=1/100, g_tol= 0.001):
        """
        ...
        for ex_right in range(self.ex_rights):
            ...
            if print_progress:
                res=scipy.optimize.minimize(objective_BFGS,x_0, jac=True, method='BFGS', args=(rel_mart_incr, series_prev_level), options={'disp': True, 'gtol': g_tol, 'maxiter': 500, 'norm': 2}, callback= lambda x: callback(x,rel_mart_incr, series_prev_level))
            else:
                res=scipy.optimize.minimize(objective_BFGS,x_0, jac=True, method='BFGS', args=(rel_mart_incr, series_prev_level), options={'disp': False, 'gtol': g_tol, 'maxiter': 500, 'norm': 2})
            # if ex_right==0:
        ...
        if calc_nonsmoothenedTrainingUB:
            ...
            theta_upperbound2=-1
        return r_res, theta_upperbound2, res.fun, solver_time, status_
