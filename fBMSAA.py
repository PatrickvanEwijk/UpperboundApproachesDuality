...
def main(d=1,print_progress=True, steps= 100, traj_est=80000, grid=100, step_inner=True, traj_test_lb=150000, traj_test_ub=10000, K_low=200, K_up=10, hurst=0.7, seed=0, mode_kaggle=False, L=1, mode_desai_BBS_BHS='desai'):
    time_training=[]
    time_testing=[]
    time_ub=[]
    ...
    Z3 = train_rng.normal(size=(steps,traj_test_ub, d))
    ...
    model_ = model_SAA(model_nn_rng, seed, input_size_lb, K_lower, steps, d, K_upper, input_size_ub, L=1, mode_kaggle=mode_kaggle)
    inner=grid
   ...
   ### Constructing family of martingale increments
    ...
    for time in range(steps):
        for paths_t, paths_t_p1, M, Z_t in [(S3[:,:time+1,:],S3[:, :time+2,:], M2_train_basisfunctions, Z3[:time, :, :]), (S4[:,:time+1,:],S4[:, :time+2,:], M2_test_basisfunctions,  Z4[:time, :, :])]:
            underlying_upperbound = paths_t[:,::-1,:]
            underlying_upperbound_next = paths_t_p1[:,::-1,:]
            reg_m_t_p1= underlying_upperbound_next.reshape(traj_test_ub,(time+2)*d)#np.hstack((paths_t_p1, cur_payoff_t_p1[:,None]))
            ...
            reg_m_inner= [np.hstack(((traj_inner[:,:,i])[:,None,:], underlying_upperbound)).reshape(traj_test_ub, (time+2)*d) for i in range(inner)] # stack underlying and previous underlying trajectory -> time t+1 : 0 trajectory used to predict continuation value.
            reg_m_inner=np.dstack(reg_m_inner).transpose(0,2,1).reshape(traj_test_ub*inner, (time+2)*d)
            
            basis_f_inner= []
            for index_start, index_end in inner_:
                slice_length = index_end-index_start
                exp_basis_func_tp1_inner= np.mean(model_.random_basis_LS(reg_m_inner[index_start*grid:index_end*grid,:], time+1).reshape(slice_length,grid, K_low), axis=1)
    ...
    payoff_process_manipulated = np.max(S3, axis=-1)*discount_f**np.arange(steps+1) # Desai et al. 2012
    if mode_desai_BBS_BHS.lower()=='bbs':
        payoff_process_manipulated[:,0] = np.abs(train_rng.normal( np.mean(stop_val_testing), 0.2,  size=(payoff_process_manipulated.shape[0]))) # Belomestny & Schoenmakers 2024.
...
if __name__=='__main__':
    for d,H in [ (1,0.2), (1,0.3), (1, 0.7)]:
        for grid in [100]:
            print(''.join(['*' for j in range(10)]), grid ,''.join(['*' for j in range(10)]))
            for i in range(1):                
                print(''.join(['-' for j in range(10)]), i , ''.join(['-' for j in range(10)]))
                list_inf=main(d, True, grid=grid, K_low=250,K_up=50, traj_est=100000, traj_test_ub=8000, traj_test_lb=50000, hurst=H, seed=i+8, steps=10, mode_desai_BBS_BHS='bbs', mode_kaggle=True)
                label_='HK LS'
                inf_cols = [d, H, '', '', '', '']
