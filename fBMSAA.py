...
def main(d=1,print_progress=True, steps= 9, T=1, traj_est=80000, grid=100, traj_test_lb=150000, traj_test_ub=10000, traj_train_ub=8000, K_low=100, K_up=100, hurst=0.7, seed=0, mode_kaggle=False, L=1, mode_desai_BBS_BHS='desai', mean_BBS=1, std_BBS=0.0, lambda_lasso=1/100, seed_traj_testingALL_add=0):
    """
   ...
   ### Constructing family of martingale increments
    ...
    for time in range(steps):
        for paths_t, paths_t_p1, M, Z_t, traj_, mode_ in [(S3[:,:time+1,:],S3[:, :time+2,:], M2_train_basisfunctions, Z3[:time, :, :], traj_train_ub, 'train'), (S4[:,:time+1,:],S4[:, :time+2,:], M2_test_basisfunctions,  Z4[:time, :, :], traj_test_ub, 'test')]:
            ...
            if mode_=='test' and seed_traj_testingALL_add!=0:
