...
def main(d=1,print_progress=True, steps= 9, T=1, traj_est=80000, grid=100, traj_test_lb=150000,traj_est_ub=8000, traj_test_ub=10000, K_low=200, K_up=100, hurst=0.7, seed=0, mode_kaggle=False, lambda_=1, L=1, p=100, seed_traj_testingALL_add=0):
    """
   ...
   ### Constructing family of martingale increments
    ...
    t0_ub = datetime.now()
    rng_test_loopSAA = np.random.RandomState(seed+seed_traj_testingALL_add)
    for time in range(steps):
        for paths_t, paths_t_p1, M, Z_t, traj_,mode_ in [(S3[:,:time+1,:],S3[:, :time+2,:], M2_train_basisfunctions, Z3[:time, :, :], traj_train_ub, 'train'), (S4[:,:time+1,:],S4[:, :time+2,:], M2_test_basisfunctions,  Z4[:time, :, :], traj_test_ub, 'test')]:
            ...
            if seed_traj_testingALL_add!=0 and mode_=='test': # Overwrite if testing phase and using new trajectories (so final run)
                z_inner=rng_test_loopSAA.randn(traj_, d,inner)
