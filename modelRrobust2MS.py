from tensorflow import keras, compat, multiply, constant_initializer
import os,sys
...
class model_():
    ...
    random_basis_LS_upper2= lambda self, x: self.model_ub_s(x, training=False).numpy()
...
class model_HaughKaugen(model_):
     ...
     def train_finallayer_continuationvalue(self, reg_m,y, time, traj, jump, ex_right=0):
        ...
        reg_mbasis_f_W= np.hstack([ self.random_basis_LS_upper(reg_m)* (jump[:, d])[:, None] for d in range(jump.shape[1])])
...
class model_Belomestny_etal(model_):
    ...
    def train_finallayer_continuationvalue(self, reg_m,y, time, traj, jump, ex_right=0):
        reg_mbasis_f= self.random_basis_LS(reg_m)
        reg_mbasis_f_W= np.hstack([ self.random_basis_LS_upper2(reg_m)* (jump[:, d])[:, None] for d in range(jump.shape[1])])
        reg_functions_all = np.hstack((reg_mbasis_f,reg_mbasis_f_W))
        linear_regression_output= linear_model.LinearRegression().fit(reg_functions_all,y)       
...
class model_SAA(model_HaughKaugen):
    ...
    def LP_multiple(self, payoff_paths, created_martingale_incrs, print_progress=True, mode_BHS=False, lasso_penalty = 1/100):
        ...
        *(mode_BHS=True) Belomestny, Hildebrand, Schoenmakers, 2017: min_psi max_i \max_t (\G_t^i-M_t^i(psi)).
        ...
        with gb.Env(params=options) as env, gb.Model(env=env) as LP:
    ...
    def LP_BELOM_BFGS_multiple(self, payoff_paths, created_martingale_incrs, lambda_=3, p=50, print_progress=True, calc_nonsmoothenedTrainingUB= False):
        ...
        def objective_BFGS(params,created_martingale_incrs_adj, series_prev_level=payoff_paths):
            ...
            jac=np.tensordot((1/n + sqrt_term_objective_var*(diff_term_var/std_term))/int_inner_term_sum, d_expterm_d_psi_div_p, axes=(0,0)).flatten()[mask_instably_close0_flattened[:K*(self.steps-ex_right)]==False]
            return fun, jac
        ...
        for ex_right in range(self.ex_rights):
            ...
            if print_progress:
                res=scipy.optimize.minimize(objective_BFGS,x_0, jac=True, method='BFGS', args=(rel_mart_incr, series_prev_level), options={'disp': True, 'gtol': 0.005, 'maxiter': 1000, 'norm': 2 }, callback= lambda x: callback(x,rel_mart_incr, series_prev_level))
            else:
                res=scipy.optimize.minimize(objective_BFGS,x_0, jac=True, method='BFGS', args=(rel_mart_incr, series_prev_level), options={'disp': False, 'gtol': 0.005, 'maxiter': 1000, 'norm': 2})
            r_opt = res.x
