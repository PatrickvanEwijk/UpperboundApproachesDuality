...
def main(d=3, L=3, print_progress=True, traj_est=80000, grid=100, mode_kaggle=False, traj_test_lb=150000, traj_test_ub=10000, K_low=200, K_up=10, S_0=110, strike=100, seed=0, step_size=None):
    time_training=[]
    time_testing=[]
    ...
    model_ = model_Belomestny_etal(model_nn_rng, seed, input_size_lb, K_lower, steps, d, K_upper, input_size_ub, L=L, mode_kaggle=mode_kaggle)
    ...
    for ex_right in range(L):
        ...
        for time in range(steps)[::-1]:
            ...
            con_val = model_.train_finallayer_continuationvalue(reg_m, y, time, traj, dWs[:,time,:], ex_right) #  reg_m,y, time, traj, jump, ex_right=0
            ...
            prev_right_c_ub = model_.prediction_conval_model1(reg_m_ub, traj_est, time, ex_right-1) if ex_right>0 else 0
            con_val_ub= model_.prediction_conval_model1(reg_m_ub, traj_est, time, ex_right)
            h_minus_c= (np.copy(stop_val_ub) - con_val_ub)
            jumps = (dW_ub[:, time, :])/(dt)
            y_combined = jumps*h_minus_c[:,None]
            _ = model_.train_finallayer_continuationvalue_upper(reg_m_ub,y_combined, time, traj_est, ex_right)

            ex_ind_ub = (cur_payoff_ub+prev_right_c_ub>con_val_ub)|(steps-time<=ex_right) # (steps-time<=ex_right): due to nonnegative payoff
            stop_val_ub = np.where(ex_ind_ub, cur_payoff_ub+prev_right_c_ub, con_val_ub) 
...
if __name__=='__main__':
    for d,s0,n_stopping_rights in [ (2,90, 1), (2, 90, 3)]:
        for grid in [100]:
            print(''.join(['*' for j in range(10)]), grid ,''.join(['*' for j in range(10)]))
            for i in range(1):                
                print(''.join(['-' for j in range(10)]), i ,  ''.join(['-' for j in range(10)]))
                list_inf=main(d, n_stopping_rights, True, grid=grid, K_low=400,K_up=200, traj_est=500000, traj_test_ub=30000, traj_test_lb=50000, S_0=s0, seed=i+8)
                label_='Belom. et al. TvR'
