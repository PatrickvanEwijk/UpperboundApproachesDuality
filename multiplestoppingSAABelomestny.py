...
time_testing=[]
time_ub=[]
def main(d=3, L=3, print_progress=True, traj_est=80000, grid=100, mode_kaggle=False, traj_test_lb=150000, traj_test_ub=10000, K_low=200, K_up=10, S_0=110, strike=100, seed=0, step_size=None, lambda_=1):
    time_training=[]
    time_testing=[]
    time_ub=[]
    ...
    for time in range(steps):
        for paths_t, paths_t_p1, M in [(S3[:,time,:],S3[:, time+1,:], M2_train_basisfunctions), (S4[:,time,:],S4[:, time+1,:], M2_test_basisfunctions)]:
            cur_payoff_t_p1 = payoff_option(paths_t_p1)*discount_f**(time+1)
            reg_m_t_p1= np.hstack((paths_t_p1, cur_payoff_t_p1[:,None]))

            traj_inner_ = np.exp( sigma*np.random.randn(traj_test_ub, d, grid)*(dt)**.5  + (r-delta_dividend-sigma**2/2)*dt)*paths_t[:,:,None]
            ...
            M[:,time, :] = model_.random_basis_LS(reg_m_t_p1) - exp_basis_func_tp1_inf_t
    

    payoff_process_manipulated = payoff_option(S3)*discount_f**np.arange(steps+1) # Desai et al. 2012
  #  payoff_process_manipulated[:,0] = np.abs(train_rng.normal( np.mean(payoff_process_manipulated[:,-1]), 4,  size=(payoff_process_manipulated.shape[0]))) # Belomestny & Schoenmakers 2024.

    r_opt, theta_ub_nonsmoothened, fun_smoothened= model_.LP_BELOM_BFGS_multiple(payoff_process_manipulated, M2_train_basisfunctions, print_progress=True, lambda_=lambda_, p=25, calc_nonsmoothenedTrainingUB=True)
...
if __name__=='__main__':
    for d,s0,n_stopping_rights, lambda_i in [ (2, 90, 2, 0), (2, 90, 2, 1/6), (2, 90, 2, 1), (2,90, 1, 1/6), (2,90, 1, 1/3), (2, 90, 1, 1), (2,90, 1, 2)]:# (2,90, 1, 0),(2, 90, 1, 1), (2,90,1, 2), (2, 90, 1, 3), (2,90, 1, 20)
        for grid in [2]:
            print(''.join(['*' for j in range(10)]), grid ,''.join(['*' for j in range(10)]))
            for i in range(1):                
                print(''.join(['-' for j in range(10)]), i ,''.join(['-' for j in range(10)]))
                list_inf = main(d, n_stopping_rights, True, grid=grid, K_low=400,K_up=50, traj_est=10000, traj_test_ub=40000, traj_test_lb=5000, S_0=s0, seed=i+8, lambda_= lambda_i) 
