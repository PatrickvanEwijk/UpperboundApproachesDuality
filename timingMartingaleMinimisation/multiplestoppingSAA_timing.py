...
time_testing=[]
time_ub=[]
def main(d=3, L=3, print_progress=True, traj_est=80000, grid=100, mode_kaggle=False, traj_test_lb=150000, traj_test_ub=10000, traj_est_ub= 20000,K_low=200, K_noise=10, S_0=110, strike=100, seed=0, mode_desai_BBS_BHS='desai', steps= 9):
    time_training=[]
    time_testing=[]
    time_ub=[]
    ...
    if mode_kaggle:
        ...
        step_size = (1000*150*50*20000)//K_low//grid//traj_test_ub
    ...
    if mode_desai_BBS_BHS.lower()=='bbs':
        ...
        payoff_process_manipulated[:,0] = np.abs(train_rng.normal( 5/6*np.mean(stop_val_testing_pilot), .25*np.std(payoff_option(S2[:,-1,:])*discount_f**stop_time),  size=(payoff_process_manipulated.shape[0]))) # Belomestny & Schoenmakers 2024.
    
    mart_family_construction_time= datetime.now()-t0_ub
    ...
    print('t-preparations MM', mart_family_construction_time)
    print(u_opt)
...
if __name__=='__main__':
    for d,s0,n_stopping_rights in [ (2, 90, 5)]:
        for grid in [200]:
            ...
            for step, k, samples_est in  [*low_dim_comb, *high_dim_comb,*high_k_dim_comb, *high_sample_low_k]:
                if (step>7 and k>120) or (step*k>samples_est) or (step>7 and k==120 and samples_est>5000) or (step==7 and (k>120 and samples_est>=10000)): # Filter these out as computation time takes too long.
                    ...
                    for i in range(3):                
                        ...
                        list_inf = main(d, n_stopping_rights, True, grid=grid, K_low=k,K_noise=20, traj_est=5, traj_test_ub=100,traj_est_ub=samples_est, traj_test_lb=10000, S_0=s0, seed=i+8, mode_desai_BBS_BHS='bbs', steps=step) 
                        label_= 'bbs none'
                        ...
                        print(''.join(['-' for j in range(10)]), i , ''.join(['-' for j in range(10)]))
                        list_inf = main(d, n_stopping_rights, True, grid=grid, K_low=k,K_noise=20, traj_est=5, traj_test_ub=100,traj_est_ub=samples_est, traj_test_lb=10000, S_0=s0, seed=i+8, mode_desai_BBS_BHS='desai',  steps=step) 
                        label_= 'desai none'
                        inf_cols = [d, s0, n_stopping_rights, step, k,  i, samples_est]
                        inf_list=utils.process_function_output(*list_inf, label_ = label_, grid= grid, info_cols=inf_cols)
                        information.append(inf_list)
                        ...
                        print(''.join(['-' for j in range(10)]), i , ''.join(['-' for j in range(10)]))
                        list_inf = main(d, n_stopping_rights, True, grid=grid, K_low=k,K_noise=20, traj_est=5, traj_test_ub=100, traj_est_ub=samples_est, traj_test_lb=10000, S_0=s0, seed=i+8, mode_desai_BBS_BHS='bhs',  steps=step) 
                        label_= 'bhs none'
                        inf_cols = [d, s0, n_stopping_rights, step, k,  i, samples_est]
                        inf_list=utils.process_function_output(*list_inf, label_ = label_, grid= grid, info_cols=inf_cols)
                        information.append(inf_list)
                ...
                # print(''.join(['-' for j in range(10)]), i , ''.join(['-' for j in range(10)]))
                # list_inf = main(d, n_stopping_rights, True, grid=grid, K_low=300,K_up=20, npca=90, traj_est=100000, traj_test_ub=10000,traj_est_ub=10000,  traj_test_lb=100000, S_0=s0, seed=i+9, mode_desai_BBS_BHS='desai') 
                # label_= 'desai pca'
                # inf_cols = [d, s0, n_stopping_rights, '', '', '', '']
                # inf_list=utils.process_function_output(*list_inf, label_ = label_, grid= grid, info_cols=inf_cols)
                # information.append(inf_list)

                # list_inf = main(d, n_stopping_rights, True, grid=grid, K_low=300,K_up=20, npca=135, traj_est=100000, traj_test_ub=10000, traj_est_ub=10000, traj_test_lb=100000, S_0=s0, seed=i+8, mode_desai_BBS_BHS='bhs') 
                # label_= 'bhs pca'
                # inf_cols = [d, s0, n_stopping_rights, '', '', '', '']
                # inf_list=utils.process_function_output(*list_inf, label_ = label_, grid= grid, info_cols=inf_cols)
                # information.append(inf_list)
